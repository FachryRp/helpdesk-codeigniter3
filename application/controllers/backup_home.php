...
class Home extends CI_Controller
{
	...
	function index()
	{
		...
		$sql_ticket_close = "SELECT COUNT(id_ticket) AS jml_ticket_close FROM ticket where status = 7";
		...
		//Data ticket User
		$sql_ticket_close_user = "SELECT COUNT(id_ticket) AS jml_ticket_close FROM ticket where status = 7 and reported='$id_user'";
		$row_ticket_close_user = $this->db->query($sql_ticket_close_user)->row();

		$sql_ticket_solved_user = "SELECT COUNT(id_ticket) AS jml_ticket_solved FROM ticket where status = 6 and reported='$id_user'";
